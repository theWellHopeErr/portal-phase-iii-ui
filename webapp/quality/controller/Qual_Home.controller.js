sap.ui.define([
	...
	return Controller.extend("Portal_Phase_III.quality.controller.Qual_Home", {
		onInit: function() {
			...
			this.getOwnerComponent().getRouter().getRoute("Target_Qual_Home").attachMatched(this.onRouteMatched, this);
		},
		onRouteMatched: function(oEvent) {
			...
			var that = this;
			oDataModel.read("/InspRelListSet?$filter=Plant eq 'SA01'", {
		...
		onSelect: function(oEvent) {
			...
			var list = this.getView().getModel("RelModel").oData.list;
		...
		_getDialog: function() {
			if (!this._oDialog) {
				this._oDialog = sap.ui.xmlfragment("Portal_Phase_III.quality.view.Qual_ResultRecordDialog", this);
		...
		onSubmit: function() {
			var oInput = this.getView().getModel("RrecDialogModel").oData;

			if (!oInput.Inspchar || !oInput.Inspoper) {
			...
			oDataModel.create("/RecordResultSet", oInput, {
				method: "POST",
				dataType: "json",
				...
				success: function(data) {
					...
					if (data && data.type === "E") {
						...
						MessageToast.show("Result Record for Inpection Lot " + oInput.Insplot + " was successfully made");
						sap.ui.core.BusyIndicator.hide();
